...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info(f"🔗 Container networking: Database at {config.database_url.split('@')[-1] if '@' in config.database_url else 'configured'}")
        logger.info(f"🔄 Database retries configured: {config.database_connection_retries} attempts with {config.database_retry_delay_seconds}s delay")
        ...
        logger.warning("⚠️ Starting with potentially degraded functionality - check health endpoints")
